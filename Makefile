# This Makefile is for the Net::ARP extension to perl.
#
...
#     AUTHOR => [q[Bastian Ballmann]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     DEFINE => q[-DLINUX]
#     NAME => q[Net::ARP]
#     OBJECT => q[ARP.o send_packet_linux.o get_mac_linux.o arp_lookup_linux.o]
#     PREREQ_PM => { Net::Pcap=>q[0] }
#     TEST_REQUIRES => { Net::Pcap=>q[0] }
#     VERSION => q[1.0.11]
...
VERSION_FROM = 
DEFINE = -DLINUX
OBJECT = ARP$(OBJ_EXT) send_packet_linux$(OBJ_EXT) get_mac_linux$(OBJ_EXT) arp_lookup_linux$(OBJ_EXT)
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
XSUBPPDIR = /usr/share/perl/5.32/ExtUtils
XSUBPP = "$(XSUBPPDIR)$(DFSEP)xsubpp"
XSUBPPRUN = $(PERLRUN) $(XSUBPP)
XSPROTOARG = 
XSUBPPDEPS = /usr/share/perl/5.32/ExtUtils/typemap /usr/share/perl/5.32/ExtUtils$(DFSEP)xsubpp
XSUBPPARGS = -typemap '/usr/share/perl/5.32/ExtUtils/typemap'
XSUBPP_EXTRA_ARGS =
...
# Net::ARP might depend on some other libraries:
...
PASTHRU = LIBPERL_A="$(LIBPERL_A)"\
	...
	LD="$(LD)"\
	PREFIX="$(PREFIX)"\
	PASTHRU_DEFINE='-DLINUX $(PASTHRU_DEFINE)'\
...
manifypods : pure_all config  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  ARP.pm $(INST_MAN3DIR)/Net::ARP.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''Bastian Ballmann'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'build_requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '  ExtUtils::MakeMaker: '\''0'\''' >> META_new.yml
	$(NOECHO) $(ECHO) '  Net::Pcap: '\''0'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Net-ARP' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: v1.0.11' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "Bastian Ballmann"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Net-ARP",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '         "requires" : {' >> META_new.json
	$(NOECHO) $(ECHO) '            "Net::Pcap" : "0"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "release_status" : "stable",' >> META_new.json
	$(NOECHO) $(ECHO) '   "version" : "v1.0.11",' >> META_new.json
...
test_static :: subdirs-test_static $(MAP_TARGET)
	PERL_DL_NONLAZY=1 "/home/basti/develop/perl/Net-ARP/$(MAP_TARGET)" $(MAP_PERLINC) "-I$(INST_LIB)" "-I$(INST_ARCHLIB)" $(TEST_FILE)
	PERL_DL_NONLAZY=1 "/home/basti/develop/perl/Net-ARP/$(MAP_TARGET)" $(MAP_PERLINC) "-MExtUtils::Command::MM" "-MTest::Harness" "-e" "undef *Test::Harness::Switches; test_harness($(TEST_VERBOSE), '$(INST_LIB)', '$(INST_ARCHLIB)')" $(TEST_FILES)
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Net-ARP" VERSION="1.0.11">' > Net-ARP.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> Net-ARP.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Bastian Ballmann</AUTHOR>' >> Net-ARP.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Net-ARP.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.32" />' >> Net-ARP.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Net-ARP.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Net-ARP.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Net-ARP.ppd
